비유 싹 빼고, 수학적 증명 과정으로 바로 들어가겠습니다. 수식을 원하셨으니 최적화 이론의 본질인 **초평면 분리 정리(Separating Hyperplane Theorem)**를 이용해 강한 쌍대성이 성립하는 이유를 증명해 드리겠습니다.

---

### 1. Primal 문제와 Dual 문제의 수학적 정의

먼저 SVM의 최적화 문제를 일반적인 형태의 **Primal 문제**로 정의합니다.
$x = (w, b)$라고 할 때,
...
**핵심:** $f(x)$는 이차형식으로 **볼록 함수(Convex Function)**이고, $g_i(x)$는 선형식(Affine)이므로 당연히 볼록 함수입니다. 따라서 이 집합 **$\mathcal{A}$는 볼록 집합(Convex Set)이 됩니다.**
...
집합 $\mathcal{A}$가 볼록 집합이고, $(0, p^*)$ 아래의 영역 $\mathcal{B} = \{ (0, t) \mid t < p^* \}$ 와 겹치지 않으므로, 수학의 **초평면 분리 정리**에 의해 두 영역을 나누는 초평면(Hyperplane)이 반드시 존재합니다.
...
여기서 **슬레이터 조건(Slater's Condition)**이 개입합니다. 슬레이터 조건이 만족된다는 것은 $\mu > 0$ 임을 보장해줍니다. (만약 $\mu = 0$ 이라면, 조건을 엄격하게 만족하는 해가 존재한다는 전제에 모순이 생깁니다.)
...
수학적으로 제약 조건이 모두 Affine인 경우, **완화된 슬레이터 조건(Refined Slater's Condition)**이 적용되어, 제약 조건을 만족하는 해가 하나라도 존재하기만 하면 무조건 $\mu > 0$ 이 보장됩니다. 
